// gdpar Path 1 -- Empirical Bayes (EB) Marginal Template (Sub-phase 8.6.B)
//
// Used by R/eb.R via cmdstanr::laplace() in Step (i) of the EB workflow
// described in v07 §11.1 (operational) and v07b §§4, 9.5 (theoretical):
//
//   Step (i):  approximate the marginal log-likelihood of theta_ref by
//              integrating out xi = (a_coef, b_coef, W_raw, sigma_*, phi)
//              via Laplace, and report:
//                - theta_ref MAP point estimate (the EB point estimator
//                  $\widehat\theta_{ref}^{EB}$ of v07 §11.1.i and v07b §2.2),
//                - the marginal covariance (used by the anti-fragility
//                  diagnostics $\kappa(H)$ of Charter §2.8 of Sub-phase 8.6).
//
// The model body is structurally identical to amm_main.stan: theta_ref
// remains a parameter; cmdstanr::laplace() approximates the joint
// posterior by a Gaussian centered at the MAP and the R-side helpers in
// R/eb.R project that Gaussian onto theta_ref (marginal MAP + marginal
// covariance) under the assumption (HIER-COMPLEX)_p of v07b §3.3.
//
// Multivariate posture (Charter §2.7 of Sub-phase 8.6).
//   8.6.B operates under K = 1, p = 1, J >= 1 (degenerate scalar single
//   slot per group). The data block declares dimension fields K_slots
//   and p_dim with default 1; the body assumes K_slots == 1 and p_dim
//   == 1 throughout. 8.6.C will extend the body by adding K > 1 and
//   p > 1 branches without rewriting the data declaration.
//
// Identical to amm_main.stan except that this banner documents the EB
// context. Keeping the body bit-identical avoids subtle codegen drift
// between the FB and EB paths and lets us reuse generate_stan_code()'s
// placeholder substitution mechanism verbatim.
